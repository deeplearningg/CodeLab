...
int main(int argc, char* argv[]) {
  ...
  kahypar_configure_context_from_file(context, "./myConfig.ini");

  const kahypar_hypernode_id_t num_vertices = 7;
  const kahypar_hyperedge_id_t num_hyperedges = 4;

  //std::unique_ptr<kahypar_hyperedge_weight_t[]> vertex_weights    = std::make_unique<kahypar_hyperedge_weight_t[]>(7);
  //std::unique_ptr<kahypar_hyperedge_weight_t[]> hyperedge_weights = std::make_unique<kahypar_hyperedge_weight_t[]>(4);
  //kahypar_hyperedge_weight_t vertex_weights[7];
  //kahypar_hyperedge_weight_t hyperedge_weights[4];
  //std::vector< kahypar_hyperedge_weight_t > hyperedge_weights(4);
  //for(size_t i = 0;i < num_vertices;i++)
    //vertex_weights[i] = i;
  // force the cut to contain hyperedge 0 and 2
  //hyperedge_weights[0] = 1;  hyperedge_weights[1] = 1;
  //hyperedge_weights[2] = 1;  hyperedge_weights[3] = 1;

  std::unique_ptr<size_t[]> hyperedge_indices = std::make_unique<size_t[]>(5);
  //std::vector< size_t > hyperedge_indices(5);
  //size_t hyperedge_indices[5];
  ...
  //std::vector< kahypar_hyperedge_id_t > hyperedges(12);
  //kahypar_hyperedge_id_t hyperedges[12];
